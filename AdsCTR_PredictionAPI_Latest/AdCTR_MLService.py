...
def predict_ad_ctr():
    ...
    new_ad_columns = ['Ad_Title', 'Ad_Display_Position', 'Ad_Type', 'Search_Key_Text',
                      'Search_Location', 'Linking_Site_URL', 'Parent_Site_URL', 'Device_Type']

    testing_features.loc[len(testing_features)] = [request.json['Ad_Title'], request.json['Ad_Display_Position'], request.json['Ad_Type'],
                                                   request.json['Search_Key_Text'],request.json['Search_Location'],request.json['Linking_Site_URL'],
                                                   request.json['Parent_Site_URL'], request.json['Device_Type']]
    #print len(testing_features)
    test_encoded_features = testing_features.apply(LabelEncoder().fit_transform)
    #print len(test_encoded_features)
    new_ad_features = test_encoded_features.ix[len(test_encoded_features) - 1]
    #print new_ad_features
